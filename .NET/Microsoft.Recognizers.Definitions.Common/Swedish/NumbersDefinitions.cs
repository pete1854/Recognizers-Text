...
namespace Microsoft.Recognizers.Definitions.Swedish
{
    ...
    public static class NumbersDefinitions
    {
      ...
      public const string MoreRegexNoNumberSucceed = @"((större|mer|högre)((?!\s+än)|\s+(än(?!(\s*\d+))))|(över)(?!(\s*\d+)))";
      public const string LessRegexNoNumberSucceed = @"((färre|lägre|mindre)((?!\s+än)|\s+(än(?!(\s*\d+))))|(under)(?!(\s*\d+)))";
      public const string EqualRegexNoNumberSucceed = @"((lika|samma)?((?!\s+(som|med))|(\s+(som|med)(?!(\s*\d+)))))";
      public static readonly string OneNumberRangeMoreRegex1 = $@"({MoreOrEqual}|{MoreRegex})\s*(den|det\s+)?(?<number1>({NumberSplitMark}.)+)";
      ...
      public static readonly string OneNumberRangeLessRegex1 = $@"({LessOrEqual}|{LessRegex})\s*(den|det\s+)?(?<number2>({NumberSplitMark}.)+)";
      ...
      public static readonly string OneNumberRangeEqualRegex = $@"{EqualRegex}\s*(den|det\s+)?(?<number1>({NumberSplitMark}.)+)";
      public static readonly string TwoNumberRangeRegex1 = $@"mellan\s*(den|det\s+)?(?<number1>({NumberSplitMark}.)+)\s*och\s*(den|det\s+)?(?<number2>({NumberSplitMark}.)+)";
      ...
      public static readonly string TwoNumberRangeRegex4 = $@"(från\s+)?(?<number1>({AllIntRegex}|{AllFloatRegex}|{AllOrdinalRegex}|{DigitsNumberRegex}))\s*{TillRegex}\s*(av\s+)?(?<number2>({AllIntRegex}|{AllFloatRegex}|{AllOrdinalRegex}|{DigitsNumberRegex}))";
