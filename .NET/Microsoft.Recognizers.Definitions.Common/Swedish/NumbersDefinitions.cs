...
namespace Microsoft.Recognizers.Definitions.Swedish
{
    ...
    public static class NumbersDefinitions
    {
      ...
      public const string TillRegex = @"(till\soch\smed|till|--|-|—|——|~|–)";
      public const string MoreRegex = @"(?:(större|greater|more|higher|larger)(\s+än)?|above|over|exceed(ed|ing)?|surpass(ed|ing)?|(?<!<|=)>)";
      public const string LessRegex = @"(?:(less|lower|mindre|fewer)(\s+än)?|below|under|(?<!>|=)<)";
      public const string EqualRegex = @"(lika(\s+(med|än))?|(?<!<|>)=)";
      public static readonly string MoreOrEqualPrefix = $@"((no\s+{LessRegex})|(at\s+least))";
      public static readonly string MoreOrEqual = $@"(?:({MoreRegex}\s+(eller)?\s+{EqualRegex})|({EqualRegex}\s+(eller)?\s+{MoreRegex})|{MoreOrEqualPrefix}(\s+(eller)?\s+{EqualRegex})?|({EqualRegex}\s+(or)?\s+)?{MoreOrEqualPrefix}|>\s*=)";
      public const string MoreOrEqualSuffix = @"((och|eller)\s+(((more|greater|higher|larger|större)((?!\s+än)|(\s+än(?!(\s*\d+)))))|((over|above)(?!\s+än))))";
      ...
      public const string LessOrEqualSuffix = @"((och|eller)\s+(less|lower|mindre|fewer)((?!\s+än)|(\s+än(?!(\s*\d+)))))";
      public const string NumberSplitMark = @"(?![,.](?!\d+))";
      public const string MoreRegexNoNumberSucceed = @"((större|greater|more|higher|larger)((?!\s+än)|\s+(än(?!(\s*\d+))))|(above|over)(?!(\s*\d+)))";
      public const string LessRegexNoNumberSucceed = @"((less|lägre|smaller|fewer)((?!\s+än)|\s+(än(?!(\s*\d+))))|(below|under)(?!(\s*\d+)))";
      public const string EqualRegexNoNumberSucceed = @"(lika?((?!\s+(med|än))|(\s+(med|än)(?!(\s*\d+)))))";
